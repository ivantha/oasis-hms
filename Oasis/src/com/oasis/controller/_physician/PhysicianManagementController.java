package com.oasis.controller._physician;
...
public class PhysicianManagementController implements Controller {
    ...
    public void refreshView() {
        ...
        for (Physician physician : PhysicianServices.getPhysicianArrayList(UIName.PHYSICIAN_MANAGEMENT)) {
        ...
        telephoneTableColumn.setCellValueFactory(param -> {
            if (param.getValue().getTelephoneArrayList().size() == 0) {
                return null;
            } else {
                return param.getValue().getTelephoneArrayList().get(0).telephoneProperty();
            }
        });

        ObservableList<PhysicianDesignation> physicianDesignationObservableList = FXCollections
                .observableList(PhysicianServices.getPhysicianDesignationArrayList(UIName.PHYSICIAN_MANAGEMENT));
    ...
    public void saveButtonOnAction(ActionEvent actionEvent) {
        ...
        PhysicianServices.updatePhysician(UIName.PHYSICIAN_MANAGEMENT, new ArrayList<>(editedPhysicianHashMap.values()));
        editedPhysicianHashMap.clear();

        PhysicianServices.deletePhysician(UIName.PHYSICIAN_MANAGEMENT, new ArrayList<>(deletedPhysicianHashMap.values()));
        deletedPhysicianHashMap.clear();
