...
public class SystemServices {
    ...
    public static void addNotification(String heading, String content, NotificationType notificationType){
        ...
        int nId = Session.notificationObservableList.size() + 1;
        Notification notification = new Notification(nId, String.valueOf(nId), content, notificationType);
        Session.notificationObservableList.add(notification);

        VirtualFlow vf = (VirtualFlow) controller.getNotificationListView().lookup(".virtual-flow");
        ListView<Notification> lv = controller.getNotificationListView();

//        vf.getCell(lv.getItems().size() - 1).setOpacity(0);

        if(!controller.isNotificationScrollVisible()) {
//            if(((Notification)vf.getCell(lv.getItems().size() - 1).getItem()).animationCount == 1) {
...
//                    f.setFromValue(0);
//                    f.setToValue(1);
//                    System.out.println("m  :  " + lv.getItems().size());
//                    System.out.println("n  :  " + vf.getCell(lv.getItems().size() - 1));
//                    f.setNode(vf.getCell(lv.getItems().size() - 1));
//                    f.setOnFinished(t -> {
//                        System.out.println("First way");
//                        ((Notification)vf.getCell(lv.getItems().size() - 1).getItem()).animationCount = 1;
//                    });
//
//                    f.play();
//                });
