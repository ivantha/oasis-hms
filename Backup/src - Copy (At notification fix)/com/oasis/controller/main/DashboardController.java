...
//        notificationListView.itemsProperty().addListener(new ChangeListener<ObservableList<Notification>>() {
//            @Override
//            public void changed(ObservableValue<? extends ObservableList<Notification>> observable, ObservableList<Notification> oldValue, ObservableList<Notification> newValue) {
//                System.out.println(oldValue.size());
//                System.out.println(newValue.size());
//            }
//        });
//        notificationListView.setVbarPolicy(ScrollPane.ScrollBarPolicy.ALWAYS);
//
            ...
            public ListCell<Notification> call(ListView<Notification> param) {
                ListCell<Notification> cell = new ListCell<Notification>(){
                    ...
                    protected void updateItem(Notification item, boolean empty) {
                        ...
                        if(item!=null && !empty){
                            setGraphic(item);
                            System.out.println(item.getnId() + "----" + this.getIndex());

                            if(getIndex() == this.getListView().getItems().size() - 1){
//                                item.animationCount++;
                                System.out.println(item.getnId() + "xxxxx" + this.getIndex() + " , " + item.animationCount);
                                ...
                                if (item.animationCount == 0) {
                                    ...
                                    p.setOnFinished(e -> {
                                        if(DashboardController.this.isNotificationScrollVisible()) {
                                            VirtualFlow vf = (VirtualFlow) DashboardController.this.getNotificationListView().lookup(".virtual-flow");
                                            ListView<Notification> lv = DashboardController.this.getNotificationListView();
...
//        SystemServices.exit();
        SystemServices.addNotification("My custom notification",
                "I just want to test out how things are working. That is all", NotificationType.INFORMATION);

//        SystemServices.addNotification("My custom notification 2",
//                "I just want to test out how things are working. That is all", NotificationType.INFORMATION);
    }
    ...
    public boolean isNotificationScrollVisible(){
        return getNotificationListView().lookup(".scroll-bar").isVisible();
    }
}